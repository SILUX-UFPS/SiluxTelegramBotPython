...
#y se usan los objetos declarados arriba
    if command == '/start':
        bot.sendMessage(chat_id, str("Comandos disponibles: \n/start - Muestra este menú \n/previo - Muestra fecha de previo \n/correo - Muestra correo de profesor \n/asesoria - Muestra asesorías de Profesor \n/materia - Muestra Información de una materia \n/informacion - Información de Ingenería de Sistemas  \n/acerca - Saludo por parte del bot \n/animo - Mensaje de animo" ))
	...
	hide_keyboard = {'hide_keyboard': True}
        ...
        bolprevio = 2
        bot.sendMessage(chat_id, str("" + nombre.encode('utf-8') +", ¿De qué profesor desea saber su correo?"), reply_markup=hide_keyboard)

    elif command == '/asesoria':
        bot.sendMessage(chat_id, str("❤️"))

    elif command == '/materia':
        bot.sendMessage(chat_id, str(nombre.encode('utf-8') + "大好き❤️"))

    elif command == '/informacion':
        bot.sendMessage(chat_id, str("Página Web - http://ingsistemas.ufps.edu.co/ \nCorreo Electrónico - ingsistemas@ufps.edu.co "))

    elif command == '/acerca':
        bot.sendMessage(chat_id, str("Hola " + nombre.encode('utf-8') +", soy el Bot de Información de Ingeniería de Sistemas de la UFPS, he sido creado por el Semillero de investigación en Linux y desarrollo de Software libre, mi misión es servir a los estudiantes, mucho gusto."))
    elif command == '/animo':
